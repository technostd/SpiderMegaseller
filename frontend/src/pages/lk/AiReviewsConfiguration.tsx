...
const AiReviewsConfiguration: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            console.log('hui1')
            loadConfig().then(() => testConnections());
        }, 1000);
    ...
    const testConnections = async () => {
        ...
        try {
            const ozonResult = await aiReviewsApi.testOzonConnection();
            setOzonStatus({
                success: ozonResult.success || false,
                message: ozonResult.status || ozonResult.error || 'Неизвестная ошибка'
            })
        } catch (error) {
            console.error('Error testing connections:', error);
            setOzonStatus({
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            {yandexGptStatus ? (
                                <div
                                    className={`flex items-center p-3 rounded-lg ${yandexGptStatus.success ? 'bg-green-50' : 'bg-red-50'}`}>
                    ...
                    {yandexGptStatus.message}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            {ozonStatus ? (
                                <div
                                    className={`flex items-center p-3 rounded-lg ${ozonStatus.success ? 'bg-green-50' : 'bg-yellow-50'}`}>
                                    {ozonStatus.success ? (
                                        <CheckCircle className="w-5 h-5 text-green-600 mr-2"/>
                                    ) : (
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span
                                        className={`font-medium ${config.premoderate ? 'text-yellow-600' : 'text-green-600'}`}>
                    {config.premoderate ? 'Премодерация' : 'Автопубликация'}
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span
                                        className="font-medium text-gray-900">{config.max_response_length} симв.</span>
                                </div>
                            </div>
                        </div>
                    </div>
